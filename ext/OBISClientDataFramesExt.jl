...
module OBISClientDataFramesExt
...
DataAPI.nrow(t::OBISClient.OBISTable) = OBISClient.nrow(t)
DataAPI.ncol(t::OBISClient.OBISTable) = OBISClient.ncol(t)
...
```julia
df = DataFrame(OBISClient.occurrence("Abra alba"; limit = 100); flatten_extra = true)
```
"""
function DataFrames.DataFrame(t::OBISClient.OBISTable; flatten_extra::Bool=false)
    ...
    for nm in names
        col = OBISClient.extra_column(t, nm)
