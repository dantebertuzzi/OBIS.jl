...
The script is [`examples/brazil.jl`](https://github.com/dantebertuzzi/OBISClient.jl/blob/main/examples/brazil.jl).
...
for id in (40017, 40016, 40015)          # North, East and South Brazil Shelf
    OBISClient.statistics(; areaid = id)
end
```
...
st = OBISClient.statistics(; geometry = cell(-46.0, -24.0))
...
fewer, typically where a cell's records come mostly from one large, taxonomically narrow
dataset. Read it as a residual: it says a cell is unusual for its effort, not how many
species live there.
...
```julia
corals = vcat((DataFrame(OBISClient.occurrence("Scleractinia"; areaid = id))
...
```julia
sharks = vcat((DataFrame(OBISClient.occurrence("Chondrichthyes"; areaid = id))
               for id in (40017, 40016, 40015))...)
