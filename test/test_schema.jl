@testset "numeric coercion" begin
    ...
    @test OceanBIS.coerce_scalar(Float64, 3) === 3.0
    @test OceanBIS.coerce_scalar(Float64, 3.5) === 3.5
    @test OceanBIS.coerce_scalar(Float64, "3.5") === 3.5
    @test OceanBIS.coerce_scalar(Float64, "not a number") === missing
    @test OceanBIS.coerce_scalar(Float64, nothing) === missing

    @test OceanBIS.coerce_scalar(Int, 7) === 7
    @test OceanBIS.coerce_scalar(Int, 7.0) === 7
    @test OceanBIS.coerce_scalar(Int, 7.5) === missing
    @test OceanBIS.coerce_scalar(Int, "7") === 7

    @test OceanBIS.coerce_scalar(Bool, true) === true
    @test OceanBIS.coerce_scalar(Bool, "true") === true
    @test OceanBIS.coerce_scalar(Bool, "false") === false
    @test OceanBIS.coerce_scalar(Bool, 1) === true
    @test OceanBIS.coerce_scalar(Bool, "maybe") === missing
end
...
@testset "timestamps are milliseconds" begin
    ...
    @test OceanBIS.coerce_epoch_ms(1432857600000) == DateTime(2015, 5, 29)
    @test OceanBIS.coerce_epoch_ms("1432857600000") == DateTime(2015, 5, 29)
    @test OceanBIS.coerce_epoch_ms(nothing) === missing
    @test OceanBIS.coerce_epoch_ms("nonsense") === missing

    @test OceanBIS.coerce_iso_datetime("2025-09-17T10:21:41.000Z") ==
        DateTime(2025, 9, 17, 10, 21, 41)
    @test OceanBIS.coerce_iso_datetime("2025-05-13 16:20:21") ==
        DateTime(2025, 5, 13, 16, 20, 21)
    @test OceanBIS.coerce_iso_datetime("2025-09-17") == DateTime(2025, 9, 17)
    @test OceanBIS.coerce_iso_datetime(nothing) === missing
end
...
@testset "flags parse to a set" begin
    ...
    @test OceanBIS.coerce_flagset(["ON_LAND", "NO_DEPTH"]) == Set(["ON_LAND", "NO_DEPTH"])
    @test OceanBIS.coerce_flagset("ON_LAND,NO_DEPTH") == Set(["ON_LAND", "NO_DEPTH"])
    @test OceanBIS.coerce_flagset("on_land") == Set(["ON_LAND"])

    # No flags is an empty set, never `missing`: it is a fact about the record, so set
    # membership works without missing-handling.
    @test OceanBIS.coerce_flagset(nothing) == Set{String}()
    @test OceanBIS.coerce_flagset([]) == Set{String}()
    @test !("ON_LAND" in OceanBIS.coerce_flagset(nothing))
end

@testset "column types are concrete" begin
    for spec in OceanBIS.OCCURRENCE_SCHEMA
        T = OceanBIS.column_type(spec)
...
@testset "a value that cannot be represented becomes missing, never an error" begin
    ...
    @test OceanBIS.coerce_scalar(Int, nothing) === missing
    @test OceanBIS.coerce_scalar(Int, [1, 2]) === missing
    ...
    @test OceanBIS.coerce_scalar(String, JSON3.read("""{"a":1}""")) === missing
    @test OceanBIS.coerce_scalar(String, JSON3.read("""[1,2]""")) === missing

    # A scalar of the wrong type is still a value, so it is rendered rather than dropped.
    @test OceanBIS.coerce_scalar(String, 42) == "42"
    @test OceanBIS.coerce_scalar(String, true) == "true"
    ...
    @test OceanBIS.coerce_epoch_ms(1e30) === missing

    @test OceanBIS.coerce_iso_datetime("not a date") === missing
    @test OceanBIS.coerce_iso_datetime("") === missing
end
...
@testset "node and institute names are lifted out of their objects" begin
    ...
    @test OceanBIS.coerce_named_list(v, :name) == ["Ocean Node", "Second"]
    ...
    @test OceanBIS.coerce_named_list(mixed, :name) == ["Kept"]

    # Absent is an empty list, not `missing`: the record has no nodes, which is a fact.
    @test OceanBIS.coerce_named_list(nothing, :name) == String[]
    @test OceanBIS.coerce_named_list(missing, :name) == String[]
end
