@testset "numeric coercion" begin
    ...
    @test OBISClient.coerce_scalar(Float64, 3) === 3.0
    @test OBISClient.coerce_scalar(Float64, 3.5) === 3.5
    @test OBISClient.coerce_scalar(Float64, "3.5") === 3.5
    @test OBISClient.coerce_scalar(Float64, "not a number") === missing
    @test OBISClient.coerce_scalar(Float64, nothing) === missing

    @test OBISClient.coerce_scalar(Int, 7) === 7
    @test OBISClient.coerce_scalar(Int, 7.0) === 7
    @test OBISClient.coerce_scalar(Int, 7.5) === missing
    @test OBISClient.coerce_scalar(Int, "7") === 7

    @test OBISClient.coerce_scalar(Bool, true) === true
    @test OBISClient.coerce_scalar(Bool, "true") === true
    @test OBISClient.coerce_scalar(Bool, "false") === false
    @test OBISClient.coerce_scalar(Bool, 1) === true
    @test OBISClient.coerce_scalar(Bool, "maybe") === missing
end
...
@testset "timestamps are milliseconds" begin
    ...
    @test OBISClient.coerce_epoch_ms(1432857600000) == DateTime(2015, 5, 29)
    @test OBISClient.coerce_epoch_ms("1432857600000") == DateTime(2015, 5, 29)
    @test OBISClient.coerce_epoch_ms(nothing) === missing
    @test OBISClient.coerce_epoch_ms("nonsense") === missing

    @test OBISClient.coerce_iso_datetime("2025-09-17T10:21:41.000Z") ==
        DateTime(2025, 9, 17, 10, 21, 41)
    @test OBISClient.coerce_iso_datetime("2025-05-13 16:20:21") ==
        DateTime(2025, 5, 13, 16, 20, 21)
    @test OBISClient.coerce_iso_datetime("2025-09-17") == DateTime(2025, 9, 17)
    @test OBISClient.coerce_iso_datetime(nothing) === missing
end
...
@testset "flags parse to a set" begin
    ...
    @test OBISClient.coerce_flagset(["ON_LAND", "NO_DEPTH"]) == Set(["ON_LAND", "NO_DEPTH"])
    @test OBISClient.coerce_flagset("ON_LAND,NO_DEPTH") == Set(["ON_LAND", "NO_DEPTH"])
    @test OBISClient.coerce_flagset("on_land") == Set(["ON_LAND"])

    # No flags is an empty set, never `missing`: it is a fact about the record, so set
    # membership works without missing-handling.
    @test OBISClient.coerce_flagset(nothing) == Set{String}()
    @test OBISClient.coerce_flagset([]) == Set{String}()
    @test !("ON_LAND" in OBISClient.coerce_flagset(nothing))
end

@testset "column types are concrete" begin
    for spec in OBISClient.OCCURRENCE_SCHEMA
        T = OBISClient.column_type(spec)
...
@testset "a value that cannot be represented becomes missing, never an error" begin
    ...
    @test OBISClient.coerce_scalar(Int, nothing) === missing
    @test OBISClient.coerce_scalar(Int, [1, 2]) === missing
    ...
    @test OBISClient.coerce_scalar(String, JSON3.read("""{"a":1}""")) === missing
    @test OBISClient.coerce_scalar(String, JSON3.read("""[1,2]""")) === missing

    # A scalar of the wrong type is still a value, so it is rendered rather than dropped.
    @test OBISClient.coerce_scalar(String, 42) == "42"
    @test OBISClient.coerce_scalar(String, true) == "true"
    ...
    @test OBISClient.coerce_epoch_ms(1e30) === missing

    @test OBISClient.coerce_iso_datetime("not a date") === missing
    @test OBISClient.coerce_iso_datetime("") === missing
end
...
@testset "node and institute names are lifted out of their objects" begin
    ...
    @test OBISClient.coerce_named_list(v, :name) == ["Ocean Node", "Second"]
    ...
    @test OBISClient.coerce_named_list(mixed, :name) == ["Kept"]

    # Absent is an empty list, not `missing`: the record has no nodes, which is a fact.
    @test OBISClient.coerce_named_list(nothing, :name) == String[]
    @test OBISClient.coerce_named_list(missing, :name) == String[]
end
