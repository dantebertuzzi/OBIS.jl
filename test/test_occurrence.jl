...
@testset "the schema does not change between queries" begin
    with_mock() do
        a = OBIS.occurrence("Abra alba"; limit=3)
        b = OBIS.occurrence(
            "Abra alba"; absence=:only, limit=3, licenses=false, check_size=false
        )
        c = OBIS.occurrence("Delphinidae"; limit=25, licenses=false, check_size=false)
        ...
        @test !isempty(OBIS.extra_names(c))
        @test length(OBIS.extra_column(c, first(OBIS.extra_names(c)))) == OBIS.nrow(c)
    end
end

@testset "empty results keep the full schema" begin
    with_mock() do
        empty = OBIS.occurrence(
            "Abra alba";
...
@testset "absence and dropped records are reachable" begin
    with_mock() do
        ...
        default = OBIS.occurrence("Abra alba"; limit=3)
...
@testset "oversized pages are rejected before the request" begin
    with_mock() do
        @test_throws OBIS.OBISValidationError OBIS.occurrence_pages(
...
@testset "single record lookup" begin
    with_mock() do
        rec = OBIS.occurrence_by_id("0001837f-afc5-45a9-a76d-b63ab73fb93c")
        @test OBIS.nrow(rec) == 1
