@testset "occurrence returns a stable typed table" begin
    with_mock() do
        ...
        @test recs isa OBISClient.OBISTable
...
@testset "the schema does not change between queries" begin
    with_mock() do
        a = OBISClient.occurrence("Abra alba"; limit=3)
        b = OBISClient.occurrence(
            "Abra alba"; absence=:only, limit=3, licenses=false, check_size=false
        )
        c = OBISClient.occurrence("Delphinidae"; limit=25, licenses=false, check_size=false)
        ...
        @test !isempty(OBISClient.extra_names(c))
        @test length(OBISClient.extra_column(c, first(OBISClient.extra_names(c)))) ==
            OBISClient.nrow(c)
    end
end
...
@testset "empty results keep the full schema" begin
    with_mock() do
        ...
        @test OBISClient.nrow(empty) == 0
        @test Tables.columnnames(empty) ==
            Tables.columnnames(OBISClient.occurrence("Abra alba"; limit=3))
    end
end

@testset "absence and dropped records are reachable" begin
    with_mock() do
        absences = OBISClient.occurrence(
            "Abra alba"; absence=:only, limit=3, licenses=false, check_size=false
        )
        @test OBISClient.nrow(absences) == 3
        @test all(x -> x === true, absences.absence)

        dropped = OBISClient.occurrence(
            "Abra alba"; dropped=:only, limit=3, licenses=false, check_size=false
        )
        @test OBISClient.nrow(dropped) == 3
        ...
        default = OBISClient.occurrence("Abra alba"; limit=3)
...
@testset "oversized pages are rejected before the request" begin
    with_mock() do
        @test_throws OBISClient.OBISValidationError OBISClient.occurrence_pages(
...
@testset "single record lookup" begin
    with_mock() do
        rec = OBISClient.occurrence_by_id("0001837f-afc5-45a9-a76d-b63ab73fb93c")
        @test OBISClient.nrow(rec) == 1
