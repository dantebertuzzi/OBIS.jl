...
@testset "an export reads into the same table an API query returns" begin
    ...
    export_table = OBIS.read_export(EXPORT_DROPPED; licenses=false)
...
@testset "absence and dropped default to the API's view" begin
    ...
    @test OBIS.nrow(OBIS.read_export(EXPORT_DROPPED; licenses=false)) == 2
    @test OBIS.nrow(OBIS.read_export(EXPORT_ABSENCE; licenses=false)) == 2

    only_dropped = OBIS.read_export(EXPORT_DROPPED; dropped=:only, licenses=false)
    @test OBIS.nrow(only_dropped) == 3
    @test all(only_dropped.dropped)

    only_absence = OBIS.read_export(EXPORT_ABSENCE; absence=:only, licenses=false)
    @test OBIS.nrow(only_absence) == 3
    @test all(only_absence.absence)

    both = OBIS.read_export(EXPORT_ABSENCE; absence=:include, licenses=false)
    @test OBIS.nrow(both) == 5
    @test count(both.absence) == 3

    @test_throws OBIS.OBISValidationError OBIS.read_export(
...
@testset "rights are joined onto export rows" begin
    ...
    with_mock() do
        table = OBIS.read_export(EXPORT_DROPPED)
        ...
        other = OBIS.read_export(EXPORT_ABSENCE)
        @test all(!ismissing, other.dataset_citation)
        @test occursin("Posidonia oceanica", other.dataset_citation[1])

        summary = OBIS.licenses(table)
        @test OBIS.nrow(summary) == 1
        @test summary.license == ["CC-BY-4.0"]
        @test sum(summary.records) == OBIS.nrow(table)
        @test all(summary.permits_redistribution)

        # A licence table can be passed in, so reading many files costs one fetch.
        reused = OBIS.export_licenses()
        @test OBIS.read_export(EXPORT_DROPPED; licenses=reused).license == table.license

        @test all(ismissing, OBIS.read_export(EXPORT_DROPPED; licenses=false).license)
    end
end
...
@testset "several exports read as one table" begin
    ...
    with_mock() do
        table = OBIS.read_export([EXPORT_DROPPED, EXPORT_ABSENCE])
        @test OBIS.nrow(table) == 4          # two ordinary records from each file
        ...
        @test sum(OBIS.licenses(table).records) == OBIS.nrow(table)
    end
end
...
@testset "export provenance is the file's date, not today" begin
    ...
    table = OBIS.read_export(EXPORT_DROPPED; licenses=false)
    meta = OBIS.metadata(table)
    @test meta.endpoint == "export/occurrence"
    @test meta.accessed == Date(unix2datetime(mtime(EXPORT_DROPPED)))
    @test meta.base_url == OBIS.config().export_url
...
@testset "the provider's own terms are opt-in" begin
    ...
    plain = OBIS.read_export(EXPORT_DROPPED; licenses=false)
    @test all(isempty, plain.extra)

    full = OBIS.read_export(EXPORT_DROPPED; licenses=false, source_terms=true)
...
@testset "limit stops the read rather than the table" begin
    @test OBIS.nrow(
        OBIS.read_export(EXPORT_ABSENCE; absence=:include, licenses=false, limit=2)
    ) == 2
end

@testset "a missing file is an error that names it" begin
    err = try
        OBIS.read_export(joinpath(FIXTURE_DIR, "no_such_export.parquet"))
        nothing
    catch e
        e
    end
    ...
    @test_throws ArgumentError OBIS.read_export(String[])
end
...
@testset "the SELECT list is built from the schema" begin
    ...
    selects = [OBIS.export_select(spec) for spec in OBIS.OCCURRENCE_SCHEMA]
    @test length(selects) == length(OBIS.OCCURRENCE_SCHEMA)

    # The four that sit at the top level of the file rather than under `interpreted`.
    @test OBIS.export_select(OBIS.OCCURRENCE_SCHEMA[1]) == "_id AS \"id\""
