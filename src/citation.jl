...
function fetch_dataset_records(meta::QueryMeta, ids::Vector{String})
    ...
    if length(missing_ids) > 50
        ...
        `OBISClient.dataset(...)` with the same filters to retrieve the metadata yourself.""" maxlog =
            1
        missing_ids = String[]
    end
    ...
    if lic_col === nothing
        throw(
            OBISValidationError(
                ...
                "and dataset results; for other endpoints, query `OBISClient.dataset(...)` with " *
                "the same filters.",
            ),
        )
    end
