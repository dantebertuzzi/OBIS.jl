...
every flag OBIS uses; `OBISClient.KNOWN_FLAGS` is the fuller reference.
...
function guard_query_size(params::QueryParams)
    ...
    println(
        io,
        "     `datasetid`. `OBISClient.statistics(...)` prices a query before you run it.",
    )
    ...
    println(
        io, "  3. Stream it with `OBISClient.occurrence_pages(...)`, which processes one"
    )
    ...
    if wants_api_only
        ...
        println(
            io,
            "     `OBISClient.download_exports(...)` fetches the GeoParquet files for the",
        )
    ...
    println(io)
    print(io, "  To proceed on the API anyway: `OBISClient.configure!(api_record_limit = ")
    print(io, estimate + 1)
